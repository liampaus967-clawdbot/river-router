#!/usr/bin/env python3
"""
Load NHDPlus V2 data into PostGIS (matches NWM COMIDs).
Uses pynhd to query the NHDPlus V2 dataset.
"""
...
def download_cell(bbox):
    """Download flowlines for a grid cell using NHDPlus V2 (Medium Res)."""
    from pynhd import WaterData
    
    try:
        # Use WaterData for NHDPlus V2 (medium resolution, matches NWM)
        wd = WaterData("nhdflowline_network")
        flw = wd.bybox(bbox, box_crs=4326)
        return flw
    except Exception as e:
        error_msg = str(e)
        if "No matched records" in error_msg or "No features" in error_msg or "empty" in error_msg.lower():
            return None  # Empty cell (ocean/land)
        print(f"    Error: {e}")
        return None
...
def load_to_db(gdf, conn):
    ...
    for idx, row in gdf.iterrows():
        ...
        # NHDPlus V2 uses 'comid' as the ID field
        comid = row.get('comid') or row.get('nhdplusid')
        if comid is None:
            continue
        try:
        ...
        # Handle ftype - in V2 it's a string, map to integer code
        ftype_map = {'StreamRiver': 460, 'ArtificialPath': 558, 'Coastline': 566, 
                     'Connector': 334, 'Canal': 336, 'Pipeline': 428, 'CanalDitch': 336}
        ftype_str = row.get('ftype')
        ftype_int = ftype_map.get(ftype_str) if isinstance(ftype_str, str) else ftype_str
        
        # Get velocity and flow - V2 uses va_ma and qa_ma for mean annual
        velocity = row.get('va_ma')
        if velocity == -9999.0 or velocity == -9998.0:
            velocity = None
        flow = row.get('qa_ma')
        if flow == -9999.0 or flow == -9998.0:
            flow = None
        
        rows.append((
            comid,
            row.get('gnis_name') if row.get('gnis_name') != ' ' else None,
            row.get('lengthkm'),
            ...
            int(float(row.get('hydroseq'))) if row.get('hydroseq') else None,
            row.get('streamorde') or row.get('streamorder'),
            row.get('slope'),
            float(row.get('minelevsmo', 0) or 0) / 100 if row.get('minelevsmo') and row.get('minelevsmo') > -9000 else None,
            float(row.get('maxelevsmo', 0) or 0) / 100 if row.get('maxelevsmo') and row.get('maxelevsmo') > -9000 else None,
            velocity,
            flow,
            ftype_int,
            row.get('fcode'),
...
def main():
    ...
    print("=" * 60)
    print("NHDPlus V2 Grid Loader (NWM Compatible)")
    print("=" * 60)
